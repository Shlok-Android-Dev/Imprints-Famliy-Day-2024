...
import com.runner.helper.ESCUtil.boldOn
import com.runner.helper.ESCUtil.boldOff
import com.runner.helper.ESCUtil.underlineWithOneDotWidthOn
import com.runner.helper.ESCUtil.underlineOff
...
class SunmiPrintHelper {
    ...
    fun printText(
        ...
        try {
            ...
            try {
                ...
                if (isUnderLine) {
                    sunmiPrinterService!!.sendRAWData(underlineWithOneDotWidthOn(), null)
                } else {
                    sunmiPrinterService!!.sendRAWData(underlineOff(), null)
                }
            }
    ...
    fun printBitmapWithOut(bitmap: Bitmap?, orientation: Int) {
        if (sunmiPrinterService == null) {
            //TODO Service disconnection processing
            return
        }
    ...
    fun printExample(context: Context) {
        ...
        try {
            ...
            val bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.sunmi)
            sunmiPrinterService!!.printBitmap(bitmap, null)
            sunmiPrinterService!!.lineWrap(1, null)
     ...
     * printing
     */
    fun showPrinterStatus(context: Context?):String {
        if (sunmiPrinterService == null) {
            //TODO Service disconnection processing
            return ""
        }
     ...
     */
    fun printOneLabel(bitmap: Bitmap?,context: Context?) {
        if (sunmiPrinterService == null) {
        ...
        try {
            ...
            sunmiPrinterService!!.enterPrinterBuffer(true)
            var prefeMainedit = context!!.getSharedPreferences("FRIENDS", Context.MODE_PRIVATE)
            if(prefeMainedit.getInt("statusPrint",0)==1)
            {
                setAlign(1)
                 printBitmapWithOut(bitmap,0)
                feedPaper()
                var prefeMainedit = context.getSharedPreferences("FRIENDS", AppCompatActivity.MODE_PRIVATE)
                val editor = prefeMainedit.edit()

                editor.putInt("statusPrint", 0)
                editor.commit()
            }
            else{
                sunmiPrinterService!!.labelLocate()
                printBitmap(bitmap, 0)
