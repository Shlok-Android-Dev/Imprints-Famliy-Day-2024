...
import java.util.*
class OnEditValuePresenter : BasePresenter<EditValueView?>() {

    fun onEditValue(activity: Context, map: HashMap<String, RequestBody>, header: HashMap<String, String>, progress: Boolean,api:String) {
        view!!.enableLoadingBar(activity, progress, activity.getResources().getString(R.string.loading))
        RuheApp.instance?.apiService?.EditUser(api,map,header)?.enqueue(object : Callback<OnspotModel?> {
