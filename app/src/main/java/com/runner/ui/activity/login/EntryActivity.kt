...
class EntryActivity : BaseActivity(), UserListAdapter.Clicklistener, RedemptionView {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        prefeMain = this@EntryActivity.getSharedPreferences("FRIENDS", MODE_PRIVATE)
        ...
        txtsearchtext.text =
            "Please enter " + prefeMain1.getString("searchfield", "")!!.replace(",", "/")
        binding!!.inputLayoutNAme!!.setHint(
            prefeMain1.getString("searchfield", "")!!.replace(",", "/")
        )
        mPrinterInfo = PrinterInfo()
        mPrinter = Printer()
        mPrinterInfo = mPrinter!!.printerInfo
        sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this@EntryActivity)
        sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this@EntryActivity)
        ...
        if (Build.VERSION.SDK_INT >= 23) {
            ...
            if (!hasPermissions(this@EntryActivity, *PERMISSIONS)) {
                ActivityCompat.requestPermissions(this@EntryActivity, PERMISSIONS, PERMISSION_ALL)
            }
        }
        ...
        findViewById<View>(R.id.flMain).setOnClickListener(View.OnClickListener {
            if (binding!!.edUnique!!.hasFocus()) {
                setupUI(findViewById(R.id.flMain), this@EntryActivity)
        ...
        findViewById<View>(R.id.ivSwitch).setOnClickListener {
            if (is_front == 1) {
                is_front = -1
                val intent = Intent(this@EntryActivity, EntryActivity::class.java)
        ...
        if (BrandName.contains("L2") || BrandName.contains(Constant.SUNMI_K2MINI) || BrandName.contains(
                Constant.SUNMI_V2_PLUS_MODEL
        ...
        if (BrandName.contains(Constant.SUNMI_K2MINI)) {
            ...
            layoutParams.width =
                1050 // Set your desired width here, either in pixels or other dimension units
            layoutParams.height =
                1050 // Set your desired width here, either in pixels or other dimension units
            ...
            binding!!.Constraitlay!!.visibility = View.VISIBLE
            val layoutParams =
                binding!!.guideline.getLayoutParams() as ConstraintLayout.LayoutParams
    ...
    override fun onEditValue(position: Int) {
        ...
        var intent = Intent(this@EntryActivity, EditValueActivity::class.java)
    ...
    */
    ...
    private fun launchOtherAppTopup() {
        ...
        //Need to register your intent filter in App2 in manifest file with same action.
        sendIntent.action =
            "com.arjs.webcontxt.kotlinactivity.CardTopUpActivity" // <packagename.login>
        val bundle = Bundle()
    ...
    private fun getUserList(searchKeyword: String) {
        ...
        binding!!.edUnique!!.setText(searchKeyword)
        if (!CheckInternetConection.isInternetConnection(this@EntryActivity)) {
    ...
    override fun onUpgradeValue(position: Int) {
        ...
        var remaining: Int = 4
        if (!Registration_TypeList!!.get(position).isEmpty() && !Registration_TypeList!!.get(position).equals("null",ignoreCase = true)) {
    ...
    var handlerRunnable: Runnable = object : Runnable {
        override fun run() {
            try {
                ...
                var prefeMain = this@EntryActivity.getSharedPreferences("FRIENDS", MODE_PRIVATE)
                val editor = prefeMain.edit()
                val PRINTER_TYPE = prefeMain.getString("PRINTER_TYPE", "")
                ...
                if (!fname_list.get(mPostionforprinting).isEmpty()) {
                    name = fname_list!!.get(mPostionforprinting) + " " + Lname_list!!.get(
                        mPostionforprinting
                    )
                } else {
                ...
                if (iscontactcardprint == 1) {
                    code = "BEGIN:VCARD\n" +
                            "VERSION:2.1\n" +
                            "N:;" + name + ";;;\n" +
                            "TEL:" + country_code_list.get(mPostionforprinting) + phone_list.get(
                        mPostionforprinting
                    ) + "\n" +
                ...
                if (isPrint!!.equals("1")) {
                    ...
                    if (PRINTER_TYPE.equals(Constant.SUNMI_K2MINI, ignoreCase = true)) {
                        ...
                        Log.e(
                            "@@ERROR0: ",
                            "123 " + JSONArray(public_array).get(mPostionforprinting)
                        )
                        var jsonArray: JSONArray = JSONArray()

                        var jsonOk: JSONObject =
                            JSONArray(public_array).get(mPostionforprinting) as JSONObject;
                        var android_id = Settings.Secure.getString(
                            this@EntryActivity.getContentResolver(),
                        ...
                        try {
                            ...
                            if (category_list.get(mPostionforprinting)
                                    .equals("Exhibitor", ignoreCase = true) || category_list.get(
                                    mPostionforprinting
                                ).equals("Speaker", ignoreCase = true)
                            ) {
                            ...
                            var prefeMain = this@EntryActivity.getSharedPreferences(
                                "FRIENDS",
                        ...
                        BrandName = BrandName(this@EntryActivity)
                        ...
                        if (PRINTER_TYPE.equals("TVS", ignoreCase = true) || PRINTER_TYPE.equals(
                            ...
                            if (!appUtils.BrandMan(this@EntryActivity).toString()
                        ...
                        if (PRINTER_TYPE.equals("TVS", ignoreCase = true) || PRINTER_TYPE.equals(
                            ...
                            var prefeMain =
                                this@EntryActivity.getSharedPreferences(
                                    "FRIENDS",
                                    Activity.MODE_PRIVATE
                                )
                            var Row1 = prefeMain.getString("ROW1", "")
        ...
        if (postListener == null) {
            ...
            postListener = object : ValueEventListener {
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    Log.e(
                        "@@new232",
                        "Change" + " dataSnapshot: " + ": mCoomandReceived" + mCoomandReceived + "mBoolean" + mBoolean
                    );
            ...
            databaseReference?.child("/PRINTING/")?.child("/${android_id}/")
    ...
    override fun OnSearchUser(model: RedemptionListModel?, errCode: Int) {
        try {
            if (model!!.getStatus()!!) {
                ...
                if (array.length() == 0) {
                    SweetAlertDialog(this@EntryActivity)
                        .setTitleText("Alert!")
                ...
                var layoutManager =
                    LinearLayoutManager(this@EntryActivity, RecyclerView.VERTICAL, false)
