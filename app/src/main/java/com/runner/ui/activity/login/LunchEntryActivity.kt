...
import com.runner.adapter.LunchUserListAdapter
...
class LunchEntryActivity : BaseActivity(), LunchUserListAdapter.Clicklistener ,RedemptionView{
    ...
    var binding: ActivityEntryDiyBinding? = null
    ...
    var guest_name_list = ArrayList<String>()
    var guest_is_lunch_Allotted = ArrayList<String>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_entry_diy)
      /*  if (BrandName.contains(Constant.SUNMI_K2MINI)) {
        ...
        prefeMain = this@LunchEntryActivity.getSharedPreferences("FRIENDS", MODE_PRIVATE)
        ...
        sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this@LunchEntryActivity)
        sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this@LunchEntryActivity)
       ...
       /* binding!!.searchManualy!!.setOnLongClickListener {
        ...
        if (Build.VERSION.SDK_INT >= 23) {
            ...
            if (!hasPermissions(this@LunchEntryActivity, *PERMISSIONS)) {
                ActivityCompat.requestPermissions(this@LunchEntryActivity, PERMISSIONS, PERMISSION_ALL)
            }
        }
       ...
       binding!!.searchManualy!!.setOnClickListener {
        ...
        findViewById<View>(R.id.flMain).setOnClickListener(View.OnClickListener {
            if (binding!!.edUnique!!.hasFocus()) {
                setupUI(findViewById(R.id.flMain), this@LunchEntryActivity)
        ...
        findViewById<View>(R.id.ivSwitch).setOnClickListener {
            if (is_front == 1) {
                is_front = -1
                val intent = Intent(this@LunchEntryActivity, LunchEntryActivity::class.java)
    ...
    override fun onEditValue(position: Int) {
       ...
       var intent = Intent(this@LunchEntryActivity,EditValueActivity::class.java)
        ...
        intent.putExtra("uniquecode",code_list.get(position).toString())
        startActivity(intent)
    }
        ...
        if (BrandName.contains(
            ...
            when (action) {
                KeyEvent.ACTION_DOWN -> {
                    //各种键值屏蔽,请根据自己的需求修改 Various key value shielding, please modify according to your needs
                    ...
                    //其他的键值,一般是扫码器数据 Other key values ​​ are usually scanner data.
...
*/
   ...
   /*//Log.e("Log1",sendIntent.resolveActivity(packageManager).toString())
    ...
    private fun getUserList(searchKeyword: String) {
        ...
        binding!!.edUnique!!.setText(searchKeyword)
        if (!CheckInternetConection.isInternetConnection(this@LunchEntryActivity)) {
            val sweetAlertDialog = SweetAlertDialog(this@LunchEntryActivity)
            sweetAlertDialog.setTitleText("Alert!!")
        ...
        if (BrandName.contains(Constant.SUNMI_K2MINI)) {
            ...
            sunmiScanner!!.setScannerListener(object : SunmiScanner.OnScannerListener {
                override fun onScanData(data: String, type: SunmiScanner.DATA_DISCRIBUTE_TYPE) {
                    ...
                    //用十六进制可以看后缀是啥  Use hexadecimal to see what the suffix is
                }
                ...
                override fun onResponseData(data: String, type: SunmiScanner.DATA_DISCRIBUTE_TYPE) {
                    ...
                    //Data type: ${type}Serial command return value [hex]: $data[${ByteUtils.str2HexString(data)}]
                      """.trimIndent()
                      )*/
                }
    ...
    override fun onUpgradeValue(position: Int) {
        ...
        var remaining : Int = 4
        if(!Registration_TypeList!!.get(position).isEmpty()){
            var guest_array = JSONArray(Registration_TypeList!!.get(position))
          remaining   = 4- guest_array.length()
        }
        ...
        intent.putExtra("id", id_list[position])
        intent.putExtra("remaining_qty", remaining)
    ...
    var handlerRunnable: Runnable = object : Runnable {
        override fun run() {
            try {
                ...
                var prefeMain = this@LunchEntryActivity.getSharedPreferences("FRIENDS", MODE_PRIVATE)
                val editor = prefeMain.edit()
                val PRINTER_TYPE = prefeMain.getString("PRINTER_TYPE", "")
                ...
                if(isPrint!!.equals("1")) {
                    ...
                    if (PRINTER_TYPE.equals(Constant.SUNMI_K2MINI, ignoreCase = true)) {
                        ...
                        var android_id = Settings.Secure.getString(
                            this@LunchEntryActivity.getContentResolver(),
                        ...
                        try {
                            ...
                            var prefeMain = this@LunchEntryActivity.getSharedPreferences("FRIENDS", Activity.MODE_PRIVATE)
                        ...
                        BrandName = BrandName(this@LunchEntryActivity)
                        ...
                        if (PRINTER_TYPE.equals("TVS", ignoreCase = true) || PRINTER_TYPE.equals(
                            ...
                            if (!appUtils.BrandMan(this@LunchEntryActivity).toString().contains("sunmi",ignoreCase = true)) {
                                runOnUiThread {
                                    Log.e("@@Comejere", "asd")
                        ...
                        if (PRINTER_TYPE.equals("TVS", ignoreCase = true) || PRINTER_TYPE.equals(
                                "TSC",
                                ignoreCase = true) || BrandName.contains(Constant.SUNMI_V2_PLUS_MODEL) || BrandName.contains(
                                Constant.SUNMI_K2MINI
                            )
                        ) {
                            ...
                            var prefeMain =
                                this@LunchEntryActivity.getSharedPreferences("FRIENDS", Activity.MODE_PRIVATE)
                            var Row1 = prefeMain.getString("ROW1", "")
                            var Row2 = prefeMain.getString("ROW2", "")
                            var Row3 = prefeMain.getString("ROW3", "")//unique_code
                            var Row4 = prefeMain.getString("ROW4", "")
...
// Create a Handler to delay the toast cancellation
    ...
    private fun addChildFirebaseListerner() {
        ...
        if(postListener==null)
        {
            ...
            var android_id = Settings.Secure.getString(this@LunchEntryActivity.getContentResolver(),
                Settings.Secure.ANDROID_ID);
    ...
    private fun ChangeData(dataSnapshot: DataSnapshot) {
        ...
        if(mBoolean)
        {
            try {
                ...
                if(obj.has("members")){
                    if(obj.isNull("members")){
                        templeList!!.add("")
                    }else {
                        templeList!!.add(obj.getString("members"))
                    }
                }else{
                    templeList!!.add("")
                }
                ...
                if(obj.has("guests")){
                    if(obj.isNull("guests")){
                        Registration_TypeList!!.add("")
                    }else {
                        Registration_TypeList!!.add(obj.getString("guests"))
                    }
                }else{
                    Registration_TypeList!!.add("")
                }
    ...
    override fun onAlloteClicked(position: Int) {
        ...
        if(relations.equals("") && guest.equals("")){
            val sweetAlertDialog = SweetAlertDialog(this@LunchEntryActivity)
            sweetAlertDialog.setTitleText("Alert!!")
                .setContentText("please select atleast one Member")
                .show()
            return
        }
    ...
    override fun onCheckboxClicked(list: String) {
        Log.e("@@",list.toString())
        relations = list.replace("[","").replace("]","").replace(" ","")
        Log.e("@@rel",relations)
    ...
    override fun onshowTicket(position: Int) {
        ...
        Glide.with(this@LunchEntryActivity)
...
//write the bytes in file
    ...
    private fun UpdateToServer(id: String, code: String) {
        ...
        if (NetworkAlertUtility.isConnectingToInternet(this)) {
            ...
            prefeMain = getSharedPreferences("FRIENDS", MODE_PRIVATE)
            var api_name : String? = "lunch_update"
    ...
    private fun Allot_ToServer(id: String, code: String) {
        ...
        if (NetworkAlertUtility.isConnectingToInternet(this)) {
            ...
            var header: HashMap<String, String> = appUtils.Takeheader(this, "")
              map["relations"] = RequestBody.create(MediaType.parse("text/plain"), relations)
              map["guests"] = RequestBody.create(MediaType.parse("text/plain"), guest)
              map["unique_code"] = RequestBody.create(MediaType.parse("text/plain"), code)
            //  map["fcm_token"] = RequestBody.create(MediaType.parse("text/plain"), mFcmToken)
            // map["Device_type"] = RequestBody.create(MediaType.parse("text/plain"), "Android")
            presenter!!.Allot1(this, map, header, true, id,code,"")
...
//write the bytes in file
    ...
    override fun OnSearchUser(model: RedemptionListModel?, errCode: Int) {
        try {
            if (model!!.getStatus()!!) {
                ...
                if(array.length()==0){
                    SweetAlertDialog(this@LunchEntryActivity)
                        .setTitleText("Alert!")
              ...
              /*      if(obj.has("first_name") && obj.has("last_name")){
                ...
                val adapter = LunchUserListAdapter(
                    this@LunchEntryActivity,
                    name_list,
                    email_list,
                    phone_list,
                ...
                var layoutManager = LinearLayoutManager(this@LunchEntryActivity,RecyclerView.VERTICAL,false)
    ...
    override fun onGuestCheckboxClicked(list: String,pos: Int,strList :String) {
        Log.e("@@",list.toString())
        guest = list.replace("[","").replace("]","").replace(" ","")
        Log.e("@@rel",relations)
        ...
        if(!strList.isEmpty()){
            ...
            guest = newArray!!.toString()
        }


        Log.e("@@guest Arr...",guest)
    }
